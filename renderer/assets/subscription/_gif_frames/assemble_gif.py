"""Assemble plan-change-success.gif from kf01-kf04 keyframes."""
from pathlib import Path
from PIL import Image

FRAMES_DIR = Path(__file__).resolve().parent
ROOT = FRAMES_DIR.parent
OUT_GIF = ROOT / "plan-change-success.gif"
OUT_PNG = ROOT / "plan-change-success-still.png"
TARGET = (400, 400)
...
def main() -> None:
    keys = [FRAMES_DIR / f"kf{i:02d}.png" for i in range(1, 5)]
    ...
    print(
        f"Wrote {OUT_GIF} ({OUT_GIF.stat().st_size} bytes), "
        f"{len(frames_q)} frames, {sum(durations)}ms"
    )
